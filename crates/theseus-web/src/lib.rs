...
//!
//! Theseus：浏览器只发 `thread/start` / `thread/resume` / `thread/list` /
//! `turn/start` / `tool/approve` / `tool/reject`，只渲染 Thread / Turn /
//! Item + delta + 一张工具确认卡；不实现 loop/log；密钥不进前端；
//! 无 MCP / 会话墙 / 插件 / compaction / 策略设置页。
//!
//! `static/` 是协议连通皮，不是桌面产品终态。Codex/dsh 密度的壳在 `theseus-desktop`。
//!
//! 连接取更薄的一侧：同进程转发 [`theseus_app_server::AppServer::handle_line_sink`]
...
impl Hub {
    ...
    /// (used in tests to pin `THESEUS_SESSIONS_DIR`).
    pub fn from_server(mut server: AppServer) -> Self {
        let _ = server.handle_line(
            r#"{"jsonrpc":"2.0","id":0,"method":"initialize","params":{"clientInfo":{"name":"theseus-web","version":"0.1.0"}}}"#,
        );
        Self {
...
mod tests {
    ...
    fn static_shell_stays_a_client() {
        ...
        assert!(html.contains("theseus-web"));
        ...
        assert!(!js.contains("THESEUS_TOOL_APPROVAL"));
