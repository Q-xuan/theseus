//! Sidecar is a real `pi-app-server` process. Shutdown must not leave orphans.
...
use pi_desktop::{locate_app_server, Sidecar};
...
fn ensure_server_bin() -> PathBuf {
    ...
    let status = Command::new("cargo")
        .args(["build", "-p", "pi-app-server", "-q"])
        .status()
        .expect("cargo build pi-app-server");
    assert!(status.success(), "cargo build -p pi-app-server");
    locate_app_server().expect("pi-app-server after build")
}
...
fn spawn_initialize_thread_start_then_shutdown_reaps_child() {
    ...
    let pid = sidecar.pid();
    assert!(process_exists(pid), "sidecar should be alive after initialize");
    ...
    while found.is_none() && std::time::Instant::now() < deadline {
        if let Ok(line) = rx.recv_timeout(Duration::from_millis(200)) {
            assert!(!line.contains("sk-"), "bridge must not echo secrets: {line}");
...
async fn preview_health_does_not_mention_secrets() {
    ...
    tokio::spawn(async move {
        let _ = pi_desktop::serve_listener(listener, serve_sidecar, rx).await;
    });
    ...
    assert!(!body.contains("PI_LLM_API_KEY"));
    sidecar.shutdown();
}
