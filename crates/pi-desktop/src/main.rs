use std::net::SocketAddr;

use pi_core::default_sessions_dir;
use pi_desktop::{key_is_set, Sidecar, DEFAULT_PREVIEW_PORT, ENV_API_KEY, ENV_PREVIEW_PORT};

fn main() {
    let mut args = std::env::args().skip(1);
    match args.next().as_deref() {
        Some("--help") | Some("-h") => print_help(),
        Some("--preview") => run_preview(),
        Some("--gui") => run_gui_or_hint(),
        Some(other) => {
            eprintln!("unknown argument: {other}");
            print_help();
            std::process::exit(2);
        }
        None => {
            #[cfg(feature = "gui")]
            pi_desktop::run_gui();
            #[cfg(not(feature = "gui"))]
            run_preview();
        }
    }
}

fn print_help() {
    eprintln!(
        "\
pi-desktop — Tauri thin shell over the pi-app-server stdio sidecar

Usage:
  pi-desktop              macOS/Windows (gui feature): native window
                          Linux / no gui: loopback preview
  pi-desktop --preview    127.0.0.1 client (stdio↔WS, transitional)
  pi-desktop --gui        native window (requires --features gui)
  pi-desktop --help

The sidecar is pi-app-server (stdio JSON-RPC). Locate order:
  1. {bin}
  2. next to this executable (packaged .app / NSIS / portable folder)
  3. target/debug|release (source-tree cargo run)

{key} is inherited from the user/system environment or the parent
process. It is not a flag, not a settings field, not a prompt,
and not sent to the WebView. No auto-update.

Preview port: ${port} or {default} (127.0.0.1 only).
",
        bin = pi_desktop::ENV_SERVER_BIN,
        key = ENV_API_KEY,
        port = ENV_PREVIEW_PORT,
        default = DEFAULT_PREVIEW_PORT,
    );
}

fn run_gui_or_hint() {
    #[cfg(feature = "gui")]
    {
        pi_desktop::run_gui();
    }
    #[cfg(not(feature = "gui"))]
    {
        eprintln!("pi-desktop was built without `gui`. On macOS/Windows:");
        eprintln!("  cargo run -p pi-desktop --features gui");
        eprintln!("  # or: cargo tauri dev   (after `cargo install tauri-cli --version '^2.0'`)");
        std::process::exit(2);
    }
}

fn run_preview() {
    let port = std::env::var(ENV_PREVIEW_PORT)
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(DEFAULT_PREVIEW_PORT);
    let addr = SocketAddr::from(([127, 0, 0, 1], port));

    let (sidecar, rx) = match Sidecar::start() {
        Ok(pair) => pair,
        Err(err) => {
            eprintln!("pi-desktop: {err}");
            std::process::exit(1);
        }
    };

    eprintln!("pi-desktop sidecar: {}", sidecar.path().display());
    eprintln!("sessions: {}", default_sessions_dir().display());
    eprintln!(
        "pi-desktop preview http://{addr}  (127.0.0.1 only; stdio↔WS bridge is transitional)"
    );
    if !key_is_set() {
        eprintln!(
            "{ENV_API_KEY} is not set; turn/start will return an error until you export it and restart. The key is never shown in the UI."
        );
    }

    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .expect("tokio");
    let serve_sidecar = sidecar.clone();
    runtime.block_on(async move {
        let server = pi_desktop::serve(addr, serve_sidecar, rx);
        tokio::select! {
            result = server => {
                if let Err(err) = result {
                    eprintln!("pi-desktop preview failed: {err}");
                }
            }
            _ = tokio::signal::ctrl_c() => {}
        }
    });
    sidecar.shutdown();
}
