...
const INIT_LINE: &str = r#"{"jsonrpc":"2.0","id":0,"method":"initialize","params":{"clientInfo":{"name":"pi-desktop","version":"0.5.0"}}}"#;
...
/// One `pi-app-server` child. JSON-RPC lines on stdin/stdout (Codex-shaped).
#[derive(Clone)]
pub struct Sidecar {
    inner: Arc<Inner>,
}
...
impl Sidecar {
    ...
    pub fn spawn(path: &Path) -> Result<(Self, Receiver<String>), SidecarError> {
        ...
        // Inherit the parent environment (including PI_LLM_API_KEY if set).
        // Never pass the key as an argument.
        debug_assert!(
            cmd.get_args().all(|a| {
                let s = a.to_string_lossy();
                !s.contains("PI_LLM") && !s.contains("sk-")
            }),
        ...
        let stdin = child.stdin.take().ok_or_else(|| {
            SidecarError::Io(std::io::Error::other("sidecar stdin missing"))
        })?;
        let stdout = child.stdout.take().ok_or_else(|| {
            SidecarError::Io(std::io::Error::other("sidecar stdout missing"))
        })?;

        let (tx, rx) = mpsc::channel::<String>();
        thread::Builder::new()
            .name("pi-sidecar-stdout".into())
            .spawn(move || {
