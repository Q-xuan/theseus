...
use crate::ENV_SERVER_BIN;

const TARGET_TRIPLE: &str = env!("PI_TARGET_TRIPLE");
...
pub enum LocateError {
    ...
    #[error(
        "pi-app-server binary not found. A packaged app should ship it next to \
         this executable. For a source tree: `cargo build -p pi-app-server` \
...
pub fn sidecar_file_names() -> Vec<String> {
    ...
    vec![
        bin_name().to_string(),
        format!("pi-app-server-{TARGET_TRIPLE}{suffix}"),
    ]
}
...
mod tests {
    ...
    fn bin_name_matches_platform() {
        let name = bin_name();
        assert!(name.starts_with("pi-app-server"));
        if cfg!(windows) {
    ...
    fn macos_bundle_dirs_include_contents_macos_and_resources() {
        let macos = PathBuf::from("/Applications/pi.app/Contents/MacOS");
    ...
    fn first_existing_prefers_plain_name_beside_exe() {
        let root = env::temp_dir().join(format!("pi-locate-{}", std::process::id()));
