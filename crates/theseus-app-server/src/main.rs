...
use theseus_llm::{LlmSeam, OpenAiChatSeam, ENV_API_KEY, ENV_BASE_URL};
use theseus_protocol::session_event_json_schema;
...
fn print_help() {
    eprintln!(
        "\
theseus-app-server — stdio JSON-RPC app-server

Usage:
  theseus-app-server              read NDJSON JSON-RPC on stdin, write on stdout
  theseus-app-server --demo       in-process thread/start + turn/start transcript
  theseus-app-server --print-schema
  theseus-app-server --help

Set {ENV_API_KEY} (required for turn/start) and optional {ENV_BASE_URL}.
Do not pass the key on the command line.

Session JSONL: ${ENV_SESSIONS_DIR}/<thread_id>.jsonl, or ${ENV_HOME}/sessions/, or ~/.theseus/sessions/ (PI_* and ~/.pi-app are temporary fallbacks).
"
    );
}
