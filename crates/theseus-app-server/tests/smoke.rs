//! Spawn the stdio binary. Without THESEUS_LLM_API_KEY, turn/start must fail cleanly.
...
fn stdio_thread_start_then_turn_start_without_key() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_theseus-app-server"))
        .env_remove("THESEUS_LLM_API_KEY")
        .env_remove("PI_LLM_API_KEY")
        .env("THESEUS_SESSIONS_DIR", &sess)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn theseus-app-server");
    ...
    for line in reader.lines() {
        ...
        if v.get("id") == Some(&Value::from(2)) {
            ...
            assert!(
                msg.contains("THESEUS_LLM_API_KEY"),
    ...
    assert_eq!(
        jsonl.lines().count(),
        1,
        "no-key turn must not grow the log"
    );
