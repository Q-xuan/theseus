...
use theseus_protocol::{DerivedMessage, EventData, SessionEvent, ToolCallRef};
...
mod tests {
    ...
    fn skips_empty_assistant_and_boundaries() {
        let events = vec![
            ev(
                0,
                EventData::TurnStart(theseus_protocol::TurnStart { turn: 1 }),
            ),
            ev(
                1,
    ...
    fn cross_step_tool_call_does_not_hang_on_previous_assistant() {
        let events = vec![
            ...
            ev(
                1,
                EventData::StepEnd(theseus_protocol::StepEnd { turn: 1, step: 1 }),
            ),
            ev(
                2,
                EventData::StepStart(theseus_protocol::StepStart { turn: 1, step: 2 }),
            ),
