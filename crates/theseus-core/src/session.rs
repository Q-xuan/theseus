...
impl Session {
    ...
    pub fn load_jsonl_with_max(
        ...
        if let Some(stem) = log.path().file_stem().and_then(|s| s.to_str()) {
            if stem != id {
...
mod tests {
    ...
    fn persist_append_reload_matches_events_and_derive() {
        ...
        let mut s =
            Session::create_persisted(meta("thr_1"), DEFAULT_MAX_STEPS_PER_TURN, &dir).unwrap();
        s.start_turn().unwrap();
        s.append(user(1, "hello")).unwrap();
        s.start_step().unwrap();
        s.append(assistant(1, 1, "world")).unwrap();
