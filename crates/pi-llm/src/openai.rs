...
use crate::error::{
    default_base_url, default_model, redact, truncate, LlmError, DEFAULT_BASE_URL, ENV_API_KEY,
};
...
";
    ...
    #[test]
    #[ignore = "hits the live PI_LLM_BASE_URL; export PI_LLM_API_KEY to run"]
    fn live_chat_completions() {
        let seam = OpenAiChatSeam::from_env();
        seam.ready().expect("PI_LLM_API_KEY");
        let out = seam
