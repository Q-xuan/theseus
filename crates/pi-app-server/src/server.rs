...
use pi_tools::{tool_definitions, LocalToolSeam, ToolContext, ToolOutput, ToolSeam};
...
impl AppServer {
    ...
    /// Override the JSONL directory (`{dir}/{thread_id}.jsonl`). Shared by stdio and `pi-web`.
...
impl AppServer {
    ...
    fn derived(&self, thread_id: &str) -> Vec<pi_protocol::DerivedMessage> {
        self.threads
            .get(thread_id)
...
mod tests {
    use super::*;
    use pi_llm::{LlmError, ScriptedRound, ScriptedSeam, ToolCallRequest};
    use pi_protocol::{DerivedMessage, ThreadItem};
    use std::fs;
    ...
    fn temp_workspace(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("pi-app-{name}-{}-{}", std::process::id(), now_ms()));
        fs::create_dir_all(&dir).unwrap();
        dir
    }
    ...
    fn thread_and_turn_emit_session_events() {
        ...
        let events: Vec<SessionEvent> = turn
            ...
            .collect();
        let projected = pi_core::project_items(&events);
    ...
    fn no_key_is_rpc_error_and_does_not_pollute_log() {
        ...
        match &turn.outgoing[0] {
            Outgoing::Response(r) => {
                ...
                assert!(msg.contains("PI_LLM_API_KEY"), "{msg}");
            }
    ...
    fn resume_on_new_server_matches_events_and_derive_then_only_appends() {
        ...
        assert_eq!(thread.turns[0].items, pi_core::project_items(&live));
