...
//!
//! `PI_TOOL_APPROVAL=auto|approve` (default `approve`). In `approve`, `write` /
//! `edit` / `bash` wait for `tool/approve` or `tool/reject`. `read` is always
//! automatic. Path fence still applies after a grant. Unanswered requests
//! expire as reject (`PI_TOOL_APPROVAL_TIMEOUT_SECS`, default 60).

use serde_json::{json, Value};
use std::time::Duration;

pub const ENV_APPROVAL: &str = "PI_TOOL_APPROVAL";
pub const ENV_APPROVAL_TIMEOUT: &str = "PI_TOOL_APPROVAL_TIMEOUT_SECS";
...
impl ApprovalMode {
    pub fn from_env() -> Self {
        match std::env::var(ENV_APPROVAL).ok().as_deref() {
            Some("auto") => Self::Auto,
            _ => Self::Approve,
        }
    }
...
pub fn timeout_from_env() -> Duration {
    let secs = std::env::var(ENV_APPROVAL_TIMEOUT)
        .ok()
