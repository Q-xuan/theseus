...
use pi_protocol::SessionEvent;
use thiserror::Error;

/// Override the sessions directory entirely (`{PI_SESSIONS_DIR}/{thread_id}.jsonl`).
pub const ENV_SESSIONS_DIR: &str = "PI_SESSIONS_DIR";
/// If `PI_SESSIONS_DIR` is unset, sessions live at `{PI_HOME}/sessions/`.
pub const ENV_HOME: &str = "PI_HOME";

const DEFAULT_DIR_NAME: &str = ".pi-app";
...
///
/// Order: `PI_SESSIONS_DIR` → `{PI_HOME}/sessions` → `~/.pi-app/sessions`.
...
/// Like [`resolve_sessions_dir`], but last-resort `/tmp/pi-app/sessions` if HOME is missing.
pub fn default_sessions_dir() -> PathBuf {
    resolve_sessions_dir().unwrap_or_else(|_| PathBuf::from("/tmp/pi-app/sessions"))
}
...
mod tests {
    ...
    fn list_session_files_sorts_by_mtime_and_skips_other_files() {
        ...
        assert_eq!(
            listed.iter().map(|f| f.thread_id.as_str()).collect::<Vec<_>>(),
            vec!["thr_b", "thr_a"]
        );
    ...
    #[test]
    fn resolve_prefers_sessions_dir_then_pi_home() {
        ...
        assert!(
            got.ends_with(".pi-app/sessions"),
            "default must be ~/.pi-app/sessions, got {}",
            got.display()
        );
    }
    ...
    fn chunk_is_never_written() {
        ...
        let err = log
            ...
            .unwrap_err();
        assert!(matches!(err, PersistError::NotHistory { found, .. } if found == "assistant/chunk"));
